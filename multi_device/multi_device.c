...
common_device_data cd_data = {
    ...
    .idevice_ddata = {
        [0] = {
            ...
            .dev_serial = "DEV_0"},
        [1] = {.buffer = dev_buffer_1, .size = DEV_SIZE_1, .perm = PERM_READ_WRITE, .dev_serial = "DEV_1"},

        [2] = {.buffer = dev_buffer_2, .size = DEV_SIZE_2, .perm = PERM_WRITE_ONLY, .dev_serial = "DEV_2"},

        [3] = {.buffer = dev_buffer_3, .size = DEV_SIZE_3, .perm = PERM_READ_ONLY, .dev_serial = "DEV_3"},
...
loff_t mdevice_lseek(struct file *filep, loff_t offset, int whence)
{
    ...
    case SEEK_END:
        ...
        if (temp > filep->f_pos + offset) return -EINVAL;
        filep->f_pos = temp;
        break;
    default:
        break;
...
ssize_t mdevice_read(struct file *filep, char __user *buff, size_t count, loff_t *fpos)
{
    ...
    if (copy_to_user(buff, idata->buffer + (*fpos), count))
        return -EFAULT;
    *fpos += count;
...
ssize_t mdevice_write(struct file *filep, const char __user *buff, size_t count, loff_t *fpos)
{
    ...
    if (copy_from_user(buff, idata->buffer + (*fpos), count))
        return -EFAULT;
    *fpos += count;
...
int mdevice_open(struct inode *dev_inode, struct file *filep)
{
    ...
    */
    ...
    idata = container_of(dev_inode->i_cdev, individual_device_data, mdevc) ;
    filep->private_data = idata;
    open_result = check_dev_permission(idata->perm, filep->f_mode);
    if (!open_result)
...
static int __init multi_device_init(void)
{
    ...
    for (i = 0; i < NUM_DEVICES; i++)
    {
        ...
        cdev_init(&(cd_data.idevice_ddata[i].mdevc), &mdev_fops);
        cd_data.idevice_ddata[i].mdevc.owner = THIS_MODULE;
        /* register the device with VFS*/
        int dev_add_status = cdev_add(&(cd_data.idevice_ddata[i].mdevc), cd_data.device_num + i, 1);
        if (dev_add_status != 0)
        {
        ...
        cd_data.dev_device = device_create(cd_data.dev_class, NULL, cd_data.device_num + i, NULL, "dev_%d", (i + 1));
...
mclass_del:
    for (; i > -1; i--)
    {
        device_destroy(cd_data.dev_class, cd_data.device_num + i);
        cdev_del(&cd_data.idevice_ddata[i].mdevc);
    }
    class_destroy(cd_data.dev_class);
unreg_chrdev : 
